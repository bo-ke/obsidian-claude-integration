...
var NoteTools = class {
  ...
  async listNotes(folder) {
    ...
    if (folder) {
      ...
      return files.filter((file) => file.path.startsWith(normalizedFolder)).map((file) => file.path);
    }
    ...
    this.addCommand({
      ...
      editorCallback: async (editor, view) => {
        if (!this.claudeClient) {
          new import_obsidian2.Notice("Please configure Claude API key in settings");
          return;
        }
        ...
        new import_obsidian2.Notice("Analyzing note...");
        try {
...
${content}`
    ...
    this.addCommand({
      ...
      editorCallback: async (editor, view) => {
        ...
        if (!selection) {
          new import_obsidian2.Notice("Please select some text");
          return;
        }
        new import_obsidian2.Notice("Improving text...");
        try {
          const response = await this.claudeClient.sendMessage([
            {
              role: "user",
...
${selection}`
          ...
          new import_obsidian2.Notice("Text improved!");
        } catch (error) {
          new import_obsidian2.Notice(`Error: ${error.message}`);
        }
      }
    ...
    this.addCommand({
      ...
      callback: () => {
        if (this.settings.enableMCPServer) {
          new import_obsidian2.Notice("MCP Server functionality requires running the separate server process. See README for instructions.");
        } else {
          new import_obsidian2.Notice("MCP Server is disabled. Enable it in settings.");
        }
      }
    });
...
var ClaudeChatModal = class extends import_obsidian2.Modal {
  plugin;
  editor;
  messages = [];
  ...
  onOpen() {
    ...
    insertButton.onclick = () => {
      ...
      if (lastAssistantMessage) {
        ...
        new import_obsidian2.Notice("Inserted to note");
        this.close();
      }
    };
    clearButton.onclick = () => {
...
var ClaudeResponseModal = class extends import_obsidian2.Modal {
  response;
...
var ClaudeIntegrationSettingTab = class extends import_obsidian2.PluginSettingTab {
  plugin;
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Claude API Key").setDesc("Enter your Anthropic API key").addText((text) => text.setPlaceholder("sk-ant-...").setValue(this.plugin.settings.apiKey).onChange(async (value) => {
      this.plugin.settings.apiKey = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Model").setDesc("Claude model to use").addDropdown((dropdown) => dropdown.addOption("claude-3-5-sonnet-20241022", "Claude 3.5 Sonnet").addOption("claude-3-5-haiku-20241022", "Claude 3.5 Haiku").addOption("claude-3-opus-20240229", "Claude 3 Opus").setValue(this.plugin.settings.model).onChange(async (value) => {
      this.plugin.settings.model = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Max Tokens").setDesc("Maximum tokens in response").addText((text) => text.setPlaceholder("4096").setValue(String(this.plugin.settings.maxTokens)).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Temperature").setDesc("Temperature for response generation (0-1)").addText((text) => text.setPlaceholder("1.0").setValue(String(this.plugin.settings.temperature)).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Enable MCP Server").setDesc("Allow Claude Desktop to access your notes via MCP").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableMCPServer).onChange(async (value) => {
      this.plugin.settings.enableMCPServer = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("MCP Server Port").setDesc("Port for MCP server (requires restart)").addText((text) => text.setPlaceholder("3000").setValue(String(this.plugin.settings.mcpServerPort)).onChange(async (value) => {
      const num = parseInt(value);
      if (!isNaN(num)) {
